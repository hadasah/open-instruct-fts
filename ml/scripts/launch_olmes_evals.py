...
def launch_eval(args_dict: dict):
    ...
    model = args_dict["model"] if "model" in args_dict else "none"
    ...
    if "task_args" in args_dict:
        ...
        for task in task_config_overrides:
            if task not in TASK_CONFIGS:
                if task in TASK_SUITE_CONFIGS:
                    for subtask in TASK_SUITE_CONFIGS[task]["tasks"]:
                        task_config_overrides[subtask] = update_nested_dict(
                            task_config_overrides.get(subtask, {}),
                            task_config_overrides[task]
                        )
                    del task_config_overrides[task]
                else:
                    raise RuntimeError(
                        f"--task-args includes task {task} which is not "
                        "in TASK_CONFIGS or TASK_SUITE_CONFIGS")
...
def main():
    ...
    if len(model_paths) > 0:
        for model_path in model_paths:
            ...
            logger.info(f"Launching eval for model path: {model_args_dict['model_path']}")
            maybe_rc = launch_eval(model_args_dict)
            if maybe_rc is not None and int(maybe_rc) != os.EX_OK:
                sys.exit(int(maybe_rc))
