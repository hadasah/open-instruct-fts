...
def main(
    ...
    model_seeds=None,
    model_steps=None,
    ft_model_sweep_names=None,
    ft_model_sweep_paths=None,
    ft_model_name_filter=None,
    command=None,
    relaunch_paths=None,
    relaunch_names=None,
    ...
    **kwargs,
    ...
    if relaunch_paths or relaunch_names:
        ...
        ft_model_sweep_names = ft_model_sweep_names.split(",") if ft_model_sweep_names else []
        ft_model_sweep_paths = ft_model_sweep_paths.split(",") if ft_model_sweep_paths else []
        if ft_model_sweep_names and not ft_model_sweep_paths:
                ft_model_sweep_paths = [os.path.join(USER_SPECS['DEFAULT_SAVE_PATH'], sweep_name) for sweep_name in ft_model_sweep_names]
        for maybe_model_sweep_path in ft_model_sweep_paths:
            for model_sweep_path in glob.glob(maybe_model_sweep_path.strip()):
                for root, dirs, files in os.walk(model_sweep_path.strip()):
                    if ft_model_name_filter and ft_model_name_filter not in root:
                        continue
                    if "config.json" in files and "pytorch_model.bin" in files:
                        model_args_list += [(root, "main")]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ft-model-sweep-paths", type=str, default="", help="Path to the fine-tuning model sweep directory.")
    parser.add_argument("--ft-model-sweep-names", type=str, default="", help="Name of the fine-tuning model sweep.")
    parser.add_argument("--ft-model-name-filter", type=str, default="", help="If specified, will only use models that contain this string in their name.")
    ...
    main(
        ...
        ft_model_sweep_names=args.ft_model_sweep_names,
        ft_model_sweep_paths=args.ft_model_sweep_paths,
        ft_model_name_filter=args.ft_model_name_filter,
